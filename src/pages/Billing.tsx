...
=======
      window.open(
        "mailto:hello@dukafy.co.ke?subject=Enterprise Plan Inquiry",
        "_blank",
      );
        ...
        const pollInterval = setInterval(async () => {
          ...
          try {
            ...
            if (confirmRes.success) {
              ...
              toast.success(
                "🎉 Subscription activated! Thank you for your payment.",
              );
            }
          } catch {
            if (attempts >= 20) {
              ...
              setPaymentLoading(false);
              toast.error(
                "Payment confirmation timed out. If you paid, please refresh the page.",
              );
            }
          }
        }, 3000);
...
=======
                ...
                DukaFy. Your data is safe and will be available once you
                reactivate.
>>>>>>> 8831112745dcd872c38f301c814f6d1906d0b0b2
              </p>
            </div>
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span
                    className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold capitalize ${statusBadge(subscription.status)}`}
                  >
            ...
            subscription.days_remaining <= 7 && (
              <div className="mt-4 flex items-center gap-2 rounded-xl bg-warning/10 px-4 py-3 text-sm text-warning">
                ...
                <p>
                  ...
                  <strong>{subscription.days_remaining} days</strong>. Upgrade
                  to keep using DukaFy.
                </p>
              </div>
            )}
>>>>>>> 8831112745dcd872c38f301c814f6d1906d0b0b2
          ...
          <div className="mt-4 grid grid-cols-3 gap-4 rounded-xl bg-muted/50 p-4">
            <div className="text-center">
              <p className="text-lg font-bold text-foreground">
                {subscription.max_shops === -1 ? "∞" : subscription.max_shops}
              </p>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {plans.map((plan, i) => {
            ...
            return (
              <div
                key={plan.id}
                className={`relative rounded-2xl border bg-card p-6 shadow-sm transition-all hover:shadow-md ${isPopular ? "border-primary ring-1 ring-primary/20" : ""}`}
              >
                {isPopular && (
      ...
      <Modal
        open={!!paymentModal}
        onClose={() => {
          if (!paymentLoading) setPaymentModal(null);
        }}
        title={
          confirmingPayment
            ? "Confirming Payment"
            : `Pay for ${paymentModal?.name} Plan`
        }
        size="sm"
        footer={
          !confirmingPayment ? (
        ...
        {confirmingPayment ? (
          <div className="flex flex-col items-center gap-4 py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              <strong>KES {paymentModal?.price_kes.toLocaleString()}</strong>{" "}
              has been sent to <strong>{mpesaPhone}</strong>
            </p>
          ...
          <div className="space-y-4 py-2">
            <div className="rounded-xl bg-muted/50 p-4 text-center">
              <p className="text-xs text-muted-foreground">
                {paymentModal?.name} Plan — Monthly
              </p>
              <p className="text-3xl font-black text-card-foreground mt-1">
                KES {paymentModal?.price_kes.toLocaleString()}
              </p>
            </div>
            ...
            <p className="text-xs text-muted-foreground text-center">
              You will receive an M-Pesa STK push on your phone. Enter your PIN
              to complete payment.
            </p>
          </div>
        )}
